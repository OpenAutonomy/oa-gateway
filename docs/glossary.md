...
| **UCI** | Universal Command and Control Interface. Supplies the message schema — message types such as `PositionReport` and `SubsystemStatus`; `repos/uci_standard`. `mpg-uci` implements a hand-built slice of the 2.5 catalog, not the full XSD. |
...
| **OWP** | OMS WebSocket Protocol. The text-frame protocol `mpg-owp` serves, with `INIT`/`SUB`/`PUB`/`MSG`/`OK`/`ERR` operations. Its grammar comes from OMSC-SPC-013, the language-agnostic CAL specification. |
| **MT** | Message type. Used in this repo for the UCI type name carried in `type_hint`, as in "the wrapper MT and the inner MT". |
| **A-GRA** | The standard defining the `MA_RxDataPayload` and `MA_TxDataPayloadCommand` wrappers that `mpg-agra` peels. Schema and interface volumes are in `repos/a-gra_standard` (`Schema/A-GRA_MessageDefinitions_v5_0_a.xsd`, and the ASK 5.0a volumes under `Documentation/`). |
...
| **PolySample** | A UCI construct whose JSON form carries a `$type` discriminator; `mpg-uci` handles it explicitly. |
...
| Term | Meaning |
|---|---|
| **STOMP** | Simple Text Oriented Messaging Protocol. A text framing over TCP that ActiveMQ accepts on `:61613`. `mpg-stomp` is a STOMP *client*, not a JMS implementation. |
