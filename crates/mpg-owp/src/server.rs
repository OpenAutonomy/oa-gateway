...
impl Default for OwpConfig {
    fn default() -> Self {
        Self {
            ...
            server_id: "mpg-0".into(),
            system_label: "MPG Prototype".into(),
...
async fn publish_owp(
    ...
    if session.config.unwrap_ma_payloads && wrapper_kind(payload.as_bytes()).is_some() {
        ...
        let hint = if mpg_uci::looks_like_xml(payload.as_bytes()) {
            mpg_uci::Message::from_xml(&payload, mpg_uci::slice::v25())
                .map(|m| m.name)
                .unwrap_or_else(|_| topic.clone())
        } else {
            type_hint_from_json(&payload).map_err(|e| e.to_string())?
        };
        let ct = if mpg_uci::looks_like_xml(payload.as_bytes()) {
            ContentType::xml()
        } else {
            ContentType::json()
        };
...
fn toward_xml(mut env: Envelope) -> Result<Envelope, String> {
    if mpg_uci::looks_like_xml(&env.payload) {
        env.content_type = ContentType::xml();
        return Ok(env);
    }
    let text = std::str::from_utf8(&env.payload).map_err(|e| e.to_string())?;
    let schema = mpg_uci::slice::v25();
    let msg = mpg_uci::Message::from_json(text, schema).map_err(|e| e.to_string())?;
...
fn xml_to_oms_json(raw: &str) -> String {
    if !mpg_uci::looks_like_xml(raw.as_bytes()) {
        return raw.to_owned();
    }
    let schema = mpg_uci::slice::v25();
    match mpg_uci::Message::from_xml(raw, schema).and_then(|m| m.to_json(schema)) {
        Ok(json) => json,
        Err(err) => {
