...
use mpg_testing::fixtures::POSITION_REPORT_XML;
use mpg_testing::stomp::{start_stomp_adapter, StompPeer};
use mpg_testing::util::{unique_token, xml_marked};
use tokio::net::TcpStream;
use tokio::time::timeout;

fn broker_addr() -> SocketAddr {
    std::env::var("MPG_ACTIVEMQ_STOMP")
        .unwrap_or_else(|_| "127.0.0.1:61613".into())
        .parse()
        .expect("MPG_ACTIVEMQ_STOMP must be host:port")
}
...
async fn inbound_does_not_echo_duplicate() {
    ...
    match timeout(Duration::from_millis(500), watcher.recv()).await {
        ...
        Ok(dup) => {
            ...
            if body.contains(&token) {
                panic!("mpg echoed inbound XML back onto the broker: {body}");
            }
        }
    }
